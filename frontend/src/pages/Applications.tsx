...
const Applications: React.FC = () => {
    ...
    const handleGenerateCoverLetter = async (app: Application) => {
        NotificationService.notify(`Generating cover letter for ${app.roleName} at ${app.companyName}…`, 'info');
        try {
            const res = await api.post(`/applications/${app.id}/cover-letter?userId=${userId}`, null, {
                responseType: 'blob',
            });
            const url = window.URL.createObjectURL(new Blob([res.data], { type: 'application/pdf' }));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `cover-letter-${app.companyName}-${app.roleName}.pdf`);
            document.body.appendChild(link);
            link.click();
            link.remove();
            window.URL.revokeObjectURL(url);
            NotificationService.notify(`Cover letter for ${app.roleName} at ${app.companyName} downloaded!`, 'success');
        } catch {
            NotificationService.notify('Failed to generate cover letter. Make sure your resume is uploaded.', 'error');
        }
    };
                ...
                <span style={{
                    ...
                    color: '#fff', padding: '3px 10px', borderRadius: 12, fontSize: 12,
                        ...
                        <td style={{ padding: 10 }}>
                            ...
                            <button onClick={() => handleGenerateCoverLetter(app)}
                                    style={{ ...btnStyle, backgroundColor: '#8e44ad' }}>Cover Letter</button>
                        </td>
                    </tr>
                ))}
                {apps.length === 0 && (
